...
Regression coverage for issue #439: several string fields in the diagnostic report schema previously had no upper length bound, allowing oversized values to pass validation and then fail (or silently truncate) at the PostgreSQL persistence layer.
Each field below should reject values that exceed its configured ``max_length`` while accepting boundary-length values.
"""
...
BOUNDED_FIELDS = [
    ...
    (PythonInfo, "version", {"path": "/usr/bin/python3"}, 20),
